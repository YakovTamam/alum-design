...
export async function sendLeadEmails(lead: Lead): Promise<void> {
  ...
  if (from && notifyTo) {
    ...
    const buttons = emailButtonRow([
      {
        href: whatsAppLink(lead.phone, `שלום ${lead.name}, מדברים מ-${SITE_NAME} לגבי הפנייה שלך באתר.`),
        label: "פתיחת וואטסאפ",
        color: "#25d366",
        textColor: "#ffffff",
      },
  ...
  if (from && lead.email) {
    ...
    try {
      const { error } = await resend.emails.send({
        from,
        to: lead.email,
        subject: `תודה שפנית אלינו — ${SITE_NAME}`,
        html: renderEmailLayout({ previewText: "קיבלנו את פנייתך ונחזור אליך בהקדם", bodyHtml }),
        text: `שלום ${lead.name},\n\nתודה שפנית ל-${SITE_NAME}. קיבלנו את פנייתך ונחזור אליך בהקדם האפשרי.\n\nבברכה,\nצוות ${SITE_NAME}`,
      });
...
export async function sendInvitationEmail(data: {
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 8px;font-size:14px;line-height:1.7;color:#4a443c;">
      הוזמנת להצטרף למערכת הניהול של ${SITE_NAME} בתפקיד <strong>${escapeHtml(roleLabel)}</strong>.
    </p>
...
export async function sendPasswordResetEmail(data: { email: string; resetUrl: string }): Promise<void> {
  ...
  const bodyHtml = `
    ...
    <p style="margin:0 0 8px;font-size:14px;line-height:1.7;color:#4a443c;">
      התקבלה בקשה לאיפוס הסיסמה לחשבון הניהול שלך ב-${SITE_NAME}.
    </p>
  ...
  try {
    const { error } = await resend.emails.send({
      ...
      html: renderEmailLayout({ previewText: "בקשה לאיפוס סיסמה לחשבון הניהול", bodyHtml }),
