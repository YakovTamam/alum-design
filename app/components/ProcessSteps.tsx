    ...
    <section className="bg-zinc-50 text-zinc-900">
      <div className="mx-auto max-w-6xl px-6 py-20 lg:px-10">
        <h2 className="text-center text-2xl font-semibold sm:text-3xl">
          מתכנון ועד התקנה — בצורה <span className="text-[#a87a3a]">פשוטה וברורה</span>
        </h2>

        <div className="relative mt-16">
          <div
            aria-hidden
            className="absolute top-7 right-[8%] left-[8%] hidden h-px bg-[repeating-linear-gradient(to_left,#cfa15c_0_8px,transparent_8px_16px)] sm:block"
          />
          <div className="grid grid-cols-2 gap-y-10 sm:grid-cols-5 sm:gap-x-4">
            {STEPS.map((s, i) => (
              <div key={s.label} className="flex flex-col items-center text-center">
                <span className="z-10 flex h-14 w-14 items-center justify-center rounded-full border border-zinc-200 bg-white text-[#a87a3a] shadow-sm">
                  <StepIcon kind={s.icon} />
                </span>
                <span className="mt-3 text-xs font-semibold text-[#a87a3a]">
                  שלב {i + 1}
                </span>
                <h3 className="mt-1 text-sm font-semibold text-zinc-900">{s.label}</h3>
                <p className="mt-1 max-w-[10rem] text-xs leading-5 text-zinc-500">
                  {s.desc}
                </p>
              </div>
            ))}
